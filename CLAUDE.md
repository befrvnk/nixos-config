...
This project uses specialized Claude Code subagents for efficient development. Each agent handles a specific phase of the workflow with isolated context for token optimization.

### Available Subagents

| Agent | Model | Purpose | When to Use |
|-------|-------|---------|-------------|
| **@plan** | Sonnet | Research codebase, create implementation plans | Start of any feature/change |
| **@code** | Sonnet | Complex implementations with validation | High/medium complexity tasks |
| **@code-quick** | Haiku | Simple changes, faster execution | Low complexity, trivial updates |
| **@test** | Haiku | Write and run tests/validation | After implementation |
...
```
1. Main Conversation
   ↓ Discuss requirements and gather context

2. @plan Agent (Sonnet)
   ↓ Research codebase, check Context7 MCP for library docs
   ↓ Create detailed implementation plan with complexity assessment
   ↓ Returns plan to main conversation

3. Review Plan (Main)
   ↓ User reviews plan, asks questions, approves or modifies
...
5. Review Implementation (Main)
   ↓ User reviews changes, asks questions, learns from implementation
   ↓ Can request modifications if needed

6. @test Agent (Haiku) - Optional
   ↓ Writes and runs tests to verify implementation
   ↓ For NixOS: typically `nix flake check` and build validation
...
#### @plan Agent
- **Always** checks Context7 MCP for library documentation first
- Falls back to WebSearch only if Context7 doesn't have the information
- Provides complexity assessment: High/Medium/Low
- Recommends @code (Sonnet) for complex or @code-quick (Haiku) for simple
- Read-only - explores but doesn't modify files
...
#### @code-quick Agent (Haiku)
- For simple, non-architectural changes
- Same validation as @code but optimized for speed
- Use for: config updates, trivial fixes, simple options
- Avoid for: new modules, architectural changes, security code
...
- Implements minimal fixes (no refactoring)
- Always verifies fix works before returning
- Documents root cause for learning
- WebSearch available for cryptic errors

#### @investigate Agent (Sonnet)
- Checks system state, services, logs after configuration changes
- Read-only investigation (doesn't fix issues)
- Verifies services are running correctly
- Analyzes logs to identify runtime issues
- Checks hardware state and resource usage
- Reports findings with evidence and recommends next steps
- Delegates to @debug if configuration fix needed
...
- **Main conversation** retains full history and user context
- **Subagents** receive only minimal, focused input:
  - @plan: Feature description + requirements
  - @code: Implementation plan (not discussion history)
...
The @plan agent has access to Context7 MCP for up-to-date library documentation:
- Automatically resolves library names to Context7 IDs
- Fetches current, accurate documentation
- More token-efficient than web scraping
- Preferred over WebSearch for library research

### Example: Feature Implementation

```
User: "I want to add window decoration customization to niri"

Main: Ask clarifying questions about requirements
  ↓
@plan: Explores niri config, checks Context7 for niri docs, returns plan
  ↓ "Complexity: Low - Recommend @code-quick"
...
Main: Review implementation, ask questions
  User: "Why use Stylix colors here?"
  Main: Explains integration approach
  ↓
@test: Validates build succeeds ✅
  ↓
...
```
User: "niri decorations aren't applying correctly"

Main: Gather context about the issue
  ↓
@debug: Reproduces, investigates, finds config typo, fixes, verifies ✅
  ↓ Returns fix summary with root cause explanation
...
```
User: Applied new configuration, verify audio is working

Main: Confirm what to check
  ↓
@investigate: Checks pipewire status, reviews audio service logs
  ↓ Returns: Services running, no errors, audio operational ✅

User: Also check if niri picked up the new keybindings
...
Use main conversation directly for:
- Quick questions about the codebase
- Clarifying requirements
- Reviewing and discussing implementations
- Making decisions about approaches
- Learning and asking "why" questions
- Iterative refinement after agent returns

### Benefits of This Workflow

✅ **Token Efficiency**: 50%+ reduction through isolated contexts
