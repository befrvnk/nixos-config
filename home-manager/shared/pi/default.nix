...
let
  ...
  piSettings = {
    ...
    inherit (cfg) defaultModel defaultProvider defaultThinkingLevel;
    hideThinkingBlock = true;
  };

  # Optional OpenRouter routing for a specific default model: always pick the
  # fastest provider. `sort: through` disables load balancing and tries
  # endpoints highest-throughput first.
  piModels = {
    providers.openrouter.modelOverrides = {
      "${cfg.openRouterFastModel}".compat.openRouterRouting = {
        sort = {
          by = "throughput";
          partition = "model";
        };
  ...
  # Only deploy models.json when the OpenRouter routing override is enabled.
  routingFile = lib.optionalAttrs cfg.openRouterFastRouting {
...
in
{
  options.pi = {
    ...
    defaultThinkingLevel = lib.mkOption {
      type = lib.types.str;
      default = "medium";
      description = "Pi default thinking level.";
    };
    openRouterFastRouting = lib.mkEnableOption "OpenRouter fast routing override: always pick the fastest provider";
    openRouterFastModel = lib.mkOption {
      type = lib.types.str;
      default = "";
      description = "OpenRouter model id that fast routing should apply to.";
    };
  };
