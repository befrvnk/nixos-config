...
- Kotlin same-workspace conflicts are detected before pi starts a second `kotlin-lsp`, so parallel sessions fail fast with a clear one-client-per-root message
