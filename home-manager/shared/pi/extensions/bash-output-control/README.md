...
Saved full-output files may contain sensitive command output. They are created exclusively with owner-only `0600` permissions, and writes honor stream backpressure so noisy commands cannot grow an unbounded in-memory write queue. Tool result details preserve Pi's built-in bash truncation shape. Files are created only when displayed output is incomplete because lines were filtered or truncated. They are not deleted by the extension; cleanup is delegated to the operating system's normal temporary-directory retention policy. You can also remove stale files manually from the directory reported by `node -p 'require("node:os").tmpdir()'`.
