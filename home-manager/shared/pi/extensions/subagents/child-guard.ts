...
function normalizePathCandidate(value: unknown, cwd: string): string | undefined {
  ...
  if (trimmed.startsWith("./") || trimmed.startsWith("../")) return path.resolve(cwd, trimmed);
  return undefined;
}
...
export function blockIfSuspiciousPath(toolName: string, input: Record<string, unknown>, cwd: string) {
...
function validateReadOnlyBashSegment(segment: string): string | undefined {
  ...
  if (command === "sed" && tokens.some((arg) => arg === "-i" || /^-i.+/.test(arg))) {
...
export function createGuardedExplorationTools(cwd: string) {
  return [
    wrapGuardedTool(createReadTool(cwd), (params) => blockIfSuspiciousPath("read", params, cwd)),
    wrapGuardedTool(createGrepTool(cwd), (params) => blockIfSuspiciousPath("grep", params, cwd)),
    wrapGuardedTool(createFindTool(cwd), (params) => blockIfSuspiciousPath("find", params, cwd)),
    wrapGuardedTool(createLsTool(cwd), (params) => blockIfSuspiciousPath("ls", params, cwd)),
    wrapGuardedTool(createBashTool(cwd), (params) => blockIfSuspiciousBashCommand(params.command, cwd)),
  ];
}
