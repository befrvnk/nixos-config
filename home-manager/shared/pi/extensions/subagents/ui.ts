...
function taskLabel(task: Pick<SubagentTaskState | SubagentTaskResult, "label" | "task">): string {
...
function describeActivity(task: SubagentTaskState): string {
  ...
  const lastOutput = truncateLine(task.recentOutputLines[task.recentOutputLines.length - 1] ?? "", 72);
...
function shouldShowFinishedTask(task: SubagentTaskState): boolean {
  if (!task.endedAt || (task.state !== "success" && task.state !== "error" && task.state !== "aborted")) return false;
  const lingerMs = isErrorState(task.state) ? ERROR_LINGER_MS : FINISHED_LINGER_MS;
  return Date.now() - task.endedAt < lingerMs;
}

function renderFinishedLine(task: SubagentTaskState, theme: Theme): string {
  const duration = task.startedAt ? formatDuration(task.startedAt, task.endedAt) : "";
  ...
  if (task.state === "error") {
    icon = theme.fg("error", "✗");
    statusText = theme.fg("error", ` error${task.error ? `: ${truncateLine(task.error, 48)}` : ""}`);
  ...
  if (task.toolUses > 0) parts.push(`${task.toolUses} tool use${task.toolUses === 1 ? "" : "s"}`);
...
function buildWidgetLines(runs: SubagentRunState[], frame: number, theme: Theme, width: number): string[] {
  ...
  const activeTasks = runs.flatMap((run) => run.tasks.filter((task) => task.state === "running"));
  const pendingTasks = runs.flatMap((run) => run.tasks.filter((task) => task.state === "pending"));
  const finishedTasks = runs.flatMap((run) => run.tasks.filter(shouldShowFinishedTask));
  ...
  const finishedLines = finishedTasks.map((task) => truncate(theme.fg("dim", "├─") + ` ${renderFinishedLine(task, theme)}`));
  const runningLines = activeTasks.map((task) => {
    const stats: string[] = [];
    const workflowName = workflowDisplayName(task.workflow);
    stats.push(formatTurns(task.turnCount));
    if (task.toolUses > 0) stats.push(`${task.toolUses} tool use${task.toolUses === 1 ? "" : "s"}`);
  ...
  const queuedLine =
    pendingTasks.length > 0
      ? truncate(theme.fg("dim", "├─") + ` ${theme.fg("muted", "◦")} ${theme.fg("dim", `${pendingTasks.length} queued`)}`)
      : undefined;
  ...
  lines.push(truncate(theme.fg("dim", "└─") + ` ${theme.fg("dim", `+${hiddenRunning + hiddenFinished} more (${overflowParts.join(", ")})`)}`));
  return lines;
}
...
export class SubagentWidget {
  ...
  update() {
    ...
    const pendingTasks = runs.flatMap((run) => run.tasks.filter((task) => task.state === "pending"));
    const finishedTasks = runs.flatMap((run) => run.tasks.filter(shouldShowFinishedTask));
    const hasAnything = activeTasks.length > 0 || pendingTasks.length > 0 || finishedTasks.length > 0;
    ...
    if (activeTasks.length > 0 || pendingTasks.length > 0) {
      ...
      if (activeTasks.length > 0) statusParts.push(`${activeTasks.length} running`);
    ...
    if (!this.widgetRegistered) {
      this.uiCtx.setWidget(
        ...
        (tui, theme) => {
          this.tui = tui;
          return {
            render: () => buildWidgetLines(this.getRuns(), this.widgetFrame, theme, tui.terminal.columns),
            invalidate: () => {
              this.widgetRegistered = false;
              this.tui = undefined;
            },
