...
See subdirectory AGENTS.md files for detailed patterns:
- `modules/AGENTS.md` - NixOS system module patterns
- `home-manager/AGENTS.md` - Home-manager organization
- `home-manager/nixos/AGENTS.md` - NixOS-specific gotchas (audio, power, theming)
- `home-manager/darwin/AGENTS.md` - Darwin-specific gotchas
- `hosts/AGENTS.md` - Host configuration patterns
...
3. AGENTS.md files - patterns and gotchas
...
- **opencode bun version:** The opencode flake has an overly strict bun version check that fails when nixpkgs' bun is older than upstream expects. Workaround in `lib/darwin.nix` patches `package.json` via `overrideAttrs` to use nixpkgs' bun version. Remove once upstream relaxes the check or nixpkgs bun catches up.
...
- **Insecure Electron:** When nixpkgs marks an Electron version as EOL/insecure, packages depending on it (e.g. `slack`, `notion-app`) break the NixOS build. Add the version to `nixpkgs.config.permittedInsecurePackages` in `modules/system/packages.nix`. Remove the permit once nixpkgs updates the dependent packages to a supported Electron version.
